...
ID3D11DepthStencilView* DeviceManager::GetDepthStencilView(PWindowRender pWR)
{
	return pWR->depthStencilView;
	return pWR->depthStencilView;
}

void DeviceManager::SetBackBufferRenderTarget(PWindowRender pWR, ID3D11DeviceContext* dc)
{
...
//	bool result;
	...
	blendStateDescription.RenderTarget[0].DestBlend = D3D11_BLEND_INV_SRC_ALPHA; //폰트 블렌딩 설정
	//blendStateDescription.RenderTarget[0].DestBlend = D3D11_BLEND_ONE; //파티클 블렌딩 설정
	...
	//if (FAILED(_device->CreateBlendState(&blendStateDescription, _alphaEnableBlendingState.GetAddressOf()))) {
	//	return false;
	//}

	if (FAILED(_device->CreateBlendState(&blendStateDescription, &_alphaEnableBlendingState)))
	{
		return false;
	}
	...
	if (FAILED(_device->CreateBlendState(&blendStateDescription, &_alphaDisableBlendingState)))
	{
		return false;
	}
...
PWindowRender DeviceManager::InitRenderScreen(HWND hWnd, ID3D11DeviceContext* dc, int screenWidth, int screenHeight, float screenNear, float screenDepth)
{
	...
	pWR->orthoMatrix		= XMMatrixOrthographicLH((float)screenWidth, (float)screenHeight, 0.0f, screenDepth);
...
XMFLOAT3 DeviceManager::ConvertScreenCoordinate(PWindowRender pWR, POINT ptWinPos, XMVECTOR cameraPos, bool isDepthBase)
{
	...
	XMFLOAT3 tmp;
	tmp.x = (float)ptWinPos.x;
	tmp.y = (float)ptWinPos.y;
	tmp.z = 0;

	XMVECTOR coord = XMVector3Unproject(XMLoadFloat3(&tmp), 0, 0, WINSIZEX, WINSIZEY, 0, 1, pWR->projectionMatrix, pWR->viewMatrix, pWR->worldMatrix);
