...
class BaseModel : public AlignedAllocationPolicy<16>
{
public:
	BaseModel();
	virtual ~BaseModel() {};
	...
	// 새로 추가: 위치
	void SetPosition(const XMFLOAT3& pos) { _center = pos; }
	void SetPosition(float x, float y, float z)
	{
		_center = XMFLOAT3(x, y, z);
	}

	// 새로 추가: 라디안 회전
	void SetRotationRad(const XMFLOAT3& rotRad) { _rotate = rotRad; }
	void SetRotationRad(float x, float y, float z)
	{
		_rotate = XMFLOAT3(x, y, z);
	}

	// 새로 추가: 도(degree) 단위 회전
	void SetRotationDeg(const XMFLOAT3& rotDeg)
	{
		const float d2r = XM_PI / 180.0f;
		_rotate = XMFLOAT3(rotDeg.x * d2r, rotDeg.y * d2r, rotDeg.z * d2r);
	}
	void SetRotationDeg(float pitchDeg, float yawDeg, float rollDeg)
	{
		const float d2r = XM_PI / 180.0f;
		_rotate = XMFLOAT3(pitchDeg * d2r, yawDeg * d2r, rollDeg * d2r);
	}
