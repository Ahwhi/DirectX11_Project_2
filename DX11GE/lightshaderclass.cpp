...
bool LightShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"./data/lightShader.hlsl");
	if(!result)
	{
		return false;
	}
	...
	XMFLOAT2 uvTiling)
{
	...
	result = SetShaderParameters(deviceContext,
		...
		lightDirection, ambientColor, dirDiffuseColor,
...
//	ID3D10Blob* toonShaderBuffer;
	...
	/*
	result = D3DCompileFromFile(fileName, NULL, NULL, "ToonPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
		&toonShaderBuffer, &errorMessage);
	if (FAILED(result))
	{
		// If the shader failed to compile it should have writen something to the error message.
		if (errorMessage)
...
/*
	// Create the pixel shader from the buffer.
	result = device->CreatePixelShader(toonShaderBuffer->GetBufferPointer(), 
		toonShaderBuffer->GetBufferSize(), NULL, &m_toonShader);
	if (FAILED(result))
	{
		return false;
	}
	...
	XMFLOAT2 uvTiling)
{
	...
	ID3D11ShaderResourceView* textures[2] = { texture, normalTexture };
	...
	dataPtr2->dirDiffuseColor = dirDiffuseColor;
	...
	for (int i = 0; i < NUM_LIGHTS; ++i)
	{
		...
		{
			dataPtr2->pointLights[i] = pointLights[i];
		}
		else
		{
			dataPtr2->pointLights[i].position = XMFLOAT3(0.f, 0.f, 0.f);
			dataPtr2->pointLights[i].range = 0.f;
			dataPtr2->pointLights[i].color = XMFLOAT4(0.f, 0.f, 0.f, 0.f);
		}
	}

	dataPtr2->uvTiling = uvTiling;
	dataPtr2->padding = XMFLOAT2(0.f, 0.f);
...
void LightShaderClass::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
	...
	if (isPhong)
		...
		deviceContext->PSSetShader(m_toonShader, NULL, 0);
...
void LightShaderClass::SetFilter(int num)
{
	D3D11_SAMPLER_DESC samplerDesc;
